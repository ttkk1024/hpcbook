...
```bash
# GCC版本特性
GCC 11.x: C++20 默认支持，Intel Sapphire Rapids 初步支持
GCC 12.x: C++23 部分支持，自动向量化增强
GCC 13.x: C++23 特性完善，OpenMP 5.2 支持，AMD Zen4 优化
GCC 14.x: C++26 实验性支持，RISC-V 向量化增强，Intel Granite Rapids 优化
...
### Intel OneAPI (ICX/IFX) 安装

#### 安装 Intel OneAPI Base & HPC Toolkit (2025版)

**重要提示**: Intel 已弃用经典编译器 (ICC/ICPC/IFORT)，全面转向基于 LLVM 的 ICX/ICPX/IFX。

**下载和安装**
```bash
# 1. 设置仓库 (Debian/Ubuntu 示例)
wget -O- https://apt.repos.intel.com/intel-gpg-keys/GPG-PUB-KEY-INTEL-SW-PRODUCTS.PUB | gpg --dearmor | sudo tee /usr/share/keyrings/oneapi-archive-keyring.gpg > /dev/null
echo "deb [signed-by=/usr/share/keyrings/oneapi-archive-keyring.gpg] https://apt.repos.intel.com/oneapi all main" | sudo tee /etc/apt/sources.list.d/oneAPI.list
sudo apt update
sudo apt install intel-oneapi-compiler-dpcpp-cpp intel-oneapi-compiler-fortran intel-oneapi-mkl
...
```bash
# 检查新版编译器 (ICX/IFX)
icx --version    # LLVM-based C Compiler
icpx --version   # LLVM-based C++ Compiler
ifx --version    # LLVM-based Fortran Compiler
```
...
**ICX/IFX 常用选项 (LLVM 风格)**
```bash
# 1. 架构优化
icx -xHost program.c           # 自动检测最佳指令集
icx -march=sapphirerapids program.c # 指定 Intel Sapphire Rapids 架构
icx -march=x86-64-v4 program.c # 通用 AVX512 支持

# 2. 数学库选项 (OneMKL)
icx -qmkl=parallel program.c    # 并行 MKL (OpenMP)
icx -qmkl=sequential program.c  # 串行 MKL

# 3. 优化报告
icx -O3 -qopt-report=3 program.c
# 注意：LLVM 报告格式与 ICC 不同，通常输出到 .optrpt 文件
```
...
```bash
# 1. 架构指定
nvcc -arch=sm_80 program.cu    # Ampere (A100)
nvcc -arch=sm_90 program.cu    # Hopper (H100)
nvcc -arch=sm_90a program.cu   # Hopper (H200/GH200 特性)
nvcc -arch=sm_100 program.cu   # Blackwell (B100/B200) [Preview]
...
### 9.4.1 高性能 GEMM 库 (CUTLASS)

CUTLASS 是 NVIDIA 开源的 CUDA C++ 模板库，用于实现高性能矩阵乘法 (GEMM) 和相关计算，是 Tensor Core 编程的首选工具。

```cpp
#include <cutlass/gemm/device/gemm.h>

// 定义 GEMM 操作类型 (FP16 累加到 FP32)
