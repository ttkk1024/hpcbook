...
gpu_diag_test() {
    ...
    # 运行 NVIDIA Field Diagnostic
    /usr/sbin/nvidia-field-diag

    # DCGM 诊断
    dcgmi diag -r 3
    # 结果：Group 0 (GPU 0) failed: Level 3 diagnostic
}
```
...
analyze_failure_patterns() {
    echo "=== 故障模式分析 ==="
    echo "1. GPU Xid 48 (Double Bit ECC Error): 15个节点"
    echo "2. GPU Xid 79 (GPU Fell Off Bus): 5个节点"
    echo "3. NVSwitch Link Flapping: 3个节点"

    echo "=== 影响范围 ==="
    echo "H100 NVLink 域: 3个"
    echo "作业中断: 12个大模型训练任务"
}
```
...
### 20.4 用于容器环境的故障 (AI/HPC)

### 案例背景
某 AI 实验室在 H100 集群上运行 PyTorch 训练任务时，频繁遭遇容器启动失败和 NCCL 通信错误，导致大规模分布式训练无法进行。

### 故障现象
#### 容器启动失败
```
错误信息：
- OCI runtime create failed: container_linux.go:380: starting container process caused: process_linux.go:545: container init caused: Running hook #0:: error running hook: exit status 1, stderr: nvidia-container-cli: requirement error: unsatisfied condition: cuda>=12.2
```

#### NCCL 通信超时
```
错误信息：
- [rank0]:[W809 10:22:33] Watchdog caught collective operation timeout: Work NCCL(AllReduce)
- NCCL WARN Connect to 10.0.0.2:12345 failed : Connection refused
```
...
# 发现：驱动版本 535.54 (支持 CUDA 12.2)，但容器镜像要求 CUDA 12.4 (驱动需 >= 550)

# 2. 检查 Enroot/Singularity 挂载
grep "nvidia" /etc/enroot/enroot.conf
```
...
```bash
# 升级 NVIDIA 驱动至 550+
dnf install kmod-nvidia-open-dkms-550.54.14
# 重启 Fabric Manager
