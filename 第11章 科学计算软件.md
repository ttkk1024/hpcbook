...
#### Python科学计算栈 (Mamba/Miniforge)

**基础环境安装 (推荐 Miniforge/Mambaforge):**

```bash
# Miniforge 安装 (自带 mamba，比 conda 更快)
...
# 科学计算包安装
mamba install numpy scipy pandas matplotlib seaborn jupyterlab

# 并行计算与 AI 框架
mamba install joblib dask ipyparallel
...
### 11.2.2 Apptainer (原 Singularity) 容器

Apptainer 是 Singularity 的开源社区版继承者，广泛用于 HPC 环境。
...
# 运行容器 (自动挂载 $HOME, /tmp 等)
srun apptainer exec scientific.sif mpirun python3 mpi_script.py

# GPU支持 (使用 --nv)
srun apptainer exec --nv scientific.sif nvidia-smi
```
...
```bash
# 从 Docker Hub 拉取并转换
apptainer pull scientific.sif docker://ubuntu:22.04

# 构建 (需 fakeroot)
apptainer build scientific.sif Singularity.def
```
...
**Spack 环境模块生成 (Lmod 集成)：**

```yaml
# spack.yaml 配置模块生成
spack:
  specs:
  - gromacs@2024 +mpi ^openmpi
  - vasp@6.4.2 +cuda ^cuda
