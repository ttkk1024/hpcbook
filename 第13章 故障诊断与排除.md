...
故障诊断是HPC运维工程师的核心技能。本章将深入介绍HPC集群中常见故障的识别、诊断和排除方法，建立系统化的故障处理流程和应急响应机制。
...
if command -v sensors &> /dev/null; then
    ...
    echo "sensors command not available" >> $LOG_FILE
fi

# 内存错误检查
echo "Memory Errors:" >> $LOG_FILE
if [ -f /var/log/mcelog ]; then
    tail -20 /var/log/mcelog >> $LOG_FILE
else
    echo "No mcelog file found" >> $LOG_FILE
fi
...
for disk in /dev/sd* /dev/nvme*; do
    if [ -b "$disk" ]; then
        ...
        smartctl -H $disk 2>/dev/null | grep -E "SMART overall|PASSED|FAILED" >> $LOG_FILE
    fi
done

# 网络接口状态
echo "Network Interfaces:" >> $LOG_FILE
ip link show | grep -E "state UP|state DOWN" >> $LOG_FILE

# 电源状态检查
echo "Power Supply:" >> $LOG_FILE
if [ -f /sys/class/power_supply/AC/online ]; then
    echo "AC Power: $(cat /sys/class/power_supply/AC/online)" >> $LOG_FILE
fi

echo "=== End Hardware Check ===" >> $LOG_FILE
echo "" >> $LOG_FILE
```
...
2. **识别中间事件**：导致顶事件的直接原因
3. **确定基本事件**：不可再分解的根本原因
4. **构建逻辑关系**：使用AND/OR门连接事件
5. **分析最小割集**：找出导致故障的最小事件组合
...
**应用日志：**
- `/var/log/slurm/` - SLURM日志
- `/var/log/httpd/` - Web服务器日志
- `/var/log/mysql/` - 数据库日志
- `/var/log/named/` - DNS服务器日志

**HPC特定日志：**
- `/var/log/lustre/` - LUSTRE文件系统日志
- `/var/log/mpi/` - MPI库日志
- `/var/log/infiniband/` - InfiniBand日志
...
| 级别 | 描述 | 响应时间 | 负责人 |
|------|------|----------|--------|
| **P1 - 紧急** | 整个集群不可用 | 15分钟 | 值班主管 |
| **P2 - 高** | 部分节点故障 | 1小时 | 高级工程师 |
| **P3 - 中** | 性能下降 | 4小时 | 工程师 |
| **P4 - 低** | 功能异常 | 24小时 | 技术支持 |
...
if [ -f /sys/class/power_supply/AC/online ]; then
    ...
    if [ "$POWER_STATUS" = "0" ]; then
        echo "⚠️ Power outage detected - check UPS and generators"
    fi
fi
...
if [ $IMPACT_PERCENT -gt 50 ]; then
    echo "⚠️ High impact detected - escalating to P1"
    # 升级到P1级别
    /opt/troubleshooting/emergency_response_p1.sh
    exit 1
fi
...
echo "- SLURM service故障 (4th Thursday)"
echo ""
...
- SLURM status: ✓ Checked (if available)
...
# CPU温度趋势
if command -v sensors &> /dev/null; then
    CPU_TEMP=$(sensors | grep -E "Package|Core" | awk '{print $4}' | sed 's/+//' | sed 's/°C//' | head -1)
    if [ ! -z "$CPU_TEMP" ]; then
        echo "$(date),CPU_TEMP,$CPU_TEMP" >> $DATA_FILE
        # 预测过热风险
        if (( $(echo "$CPU_TEMP > 75" | bc -l) )); then
            echo "⚠️ Warning: High CPU temperature detected ($CPU_TEMP°C)"
    ...
    if [ -b "$disk" ]; then
        ...
        if [ ! -z "$REALLOCATED" ] && [ "$REALLOCATED" != "0" ]; then
            ...
            if [ "$REALLOCATED" -gt 100 ]; then
                echo "⚠️ Warning: High reallocated sectors on $disk ($REALLOCATED)"
...
if [ "$ALERT_COUNT" -gt 10 ]; then
    echo "⚠️ Warning: High failure rate in the last 24 hours ($ALERT_COUNT alerts)"
...
故障诊断与排除是HPC运维工程师的核心能力，通过本章学习，你应该能够：
...
故障处理的关键在于**快速响应、准确诊断、有效解决、预防复发**。建立完善的故障处理体系，可以大大提高HPC集群的可用性和用户满意度。

## 📝 练习题

1. **实践题**：在测试环境中模拟各种故障场景并练习诊断流程
2. **配置题**：为你的集群配置完整的监控和告警系统
3. **分析题**：分析一周的系统日志，识别潜在问题
4. **设计题**：设计一个包含应急响应流程的故障处理手册

## 🔗 参考资源

- [Linux故障排除指南](https://www.kernel.org/doc/html/latest/admin-guide/bug-hunting.html)
- [SLURM故障排除](https://slurm.schedmd.com/troubleshoot.html)
- [LUSTRE故障排除](https://wiki.lustre.org/Lustre_Operations_Guide_for_Lustre_2.12)
- [Nagios故障排除](https://assets.nagios.com/downloads/nagioscore/docs/nagioscore/4/en/troubleshooting.html)