...
安全管理是HPC运维的重要组成部分。本章将深入介绍HPC集群的安全加固方法、访问控制策略、安全审计监控和漏洞管理，帮助你建立完善的安全防护体系，保障集群的安全稳定运行。
...
# 4. 配置SSH安全 (Modern Crypto)
echo "4. Configuring SSH security..."
cat > /etc/ssh/sshd_config << 'EOF'
# SSH Security Configuration (Modern)
Port 22
Protocol 2
PermitRootLogin no
...
# 2. 集中式身份认证 (FreeIPA/Kerberos)
echo "2. Configuring FreeIPA Client..."
# 假设 ipa-server 已就绪
# ipa-client-install --mkhomedir --enable-dns-updates --no-ntp -p admin -w secret
# 配置 Kerberos 仅允许 SSO
...
#### 15.2.3 容器安全 (Rootless)

**Apptainer/Podman 安全配置：**
...
# 1. 云原生运行时安全 (Falco)
echo "1. Installing Falco for runtime security..."
rpm --import https://falco.org/repo/falcosecurity-3672BA8F.asc
curl -s -o /etc/yum.repos.d/falcosecurity.repo https://falco.org/repo/falcosecurity-rpm.repo
yum install -y falco

# 配置 Falco 规则 (监控敏感文件读取)
cat >> /etc/falco/falco_rules.local.yaml << 'EOF'
- rule: Unauthorized HPC Config Read
  desc: Detect access to Slurm/Munge keys
  condition: open_read and (fd.name startswith /etc/slurm or fd.name startswith /etc/munge) and not user.name in (root, slurm, munge)
  output: "Sensitive HPC config read (user=%user.name file=%fd.name command=%proc.cmdline)"
  priority: WARNING
EOF
...
安全管理是HPC运维的重要组成部分，通过本章学习，你应该能够：

- ✅ 实施系统安全加固（操作系统、网络、服务）
- ✅ 配置有效的访问控制策略（身份验证、权限管理）
- ✅ 建立完善的安全审计和监控体系
- ✅ 实施系统的漏洞管理流程
- ✅ 处理安全事件和应急响应

安全是一个持续的过程，需要结合技术手段、管理流程和人员培训，建立全方位的安全防护体系。

## 📝 练习题

1. **实践题**：为你的集群实施完整的系统安全加固
2. **配置题**：配置基于角色的访问控制和多因素认证
3. **监控题**：部署安全监控和审计系统
4. **应急题**：模拟安全事件并练习应急响应流程
