...
系统监控是HPC运维的核心技能之一。本章将深入介绍HPC集群的监控体系构建，包括监控工具选择、性能指标收集、告警系统配置和可视化监控面板搭建。
...
define hostgroup{
    ...
    alias           Compute Nodes
    members         compute-node-01,compute-node-02,compute-node-03
}
```
...
| 工具 | 优点 | 缺点 | 适用场景 |
|------|------|------|----------|
| **Nagios** | 成熟稳定，插件丰富 | 配置复杂，可视化弱 | 基础监控，服务检查 |
| **Zabbix** | 功能全面，可视化好 | 资源消耗大 | 企业级监控 |
| **Prometheus** | 云原生，灵活 | 学习曲线陡 | 容器化环境 |
| **Ganglia** | HPC专用，轻量 | 功能相对简单 | 大规模集群 |
...
if [ "$ALERT_LEVEL" = "CRITICAL" ]; then
    curl -X POST -H 'Content-type: application/json' \
        --data "{\"text\":\"🚨 [$ALERT_LEVEL] $MESSAGE on $HOSTNAME\"}" \
        $SLACK_WEBHOOK
elif [ "$ALERT_LEVEL" = "WARNING" ]; then
    curl -X POST -H 'Content-type: application/json' \
        --data "{\"text\":\"⚠️ [$ALERT_LEVEL] $MESSAGE on $HOSTNAME\"}" \
        $SLACK_WEBHOOK
fi
...
def calculate_cluster_health():
    ...
    load_score = max(0, 100 - (max_load * 10))  # 负载权重较低
...
系统监控是HPC运维的核心技能，通过本章学习，你应该能够：

- ✅ 部署和配置主流监控工具（Nagios、Zabbix）
- ✅ 收集和分析关键性能指标
- ✅ 配置有效的告警策略
- ✅ 构建直观的可视化监控面板
- ✅ 设计集群级监控体系

监控系统的建设是一个持续优化的过程，需要根据实际运行情况不断调整和完善。

## 📝 练习题

1. **实践题**：在你的环境中部署Nagios监控系统
2. **配置题**：为计算节点配置CPU和内存告警
3. **分析题**：分析一周的监控数据，识别性能瓶颈
4. **设计题**：设计一个包含20个节点的集群监控方案
