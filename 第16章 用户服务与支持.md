...
echo ""
echo "创建目录: mkdir my_project"
mkdir -p ~/my_project
ls -la

echo ""
echo "查看文件内容: cat /etc/hostname"
cat /etc/hostname

# 3. 文件传输演示
echo ""
echo "3. 文件传输"
echo "上传文件: scp local_file.txt user@cluster:/home/user/"
echo "下载文件: scp user@cluster:/home/user/remote_file.txt ."

# 4. 作业提交演示
echo ""
echo "4. 作业提交演示"
echo "编写作业脚本: nano job_script.sh"
cat > job_script.sh << 'JOBEOF'
#!/bin/bash
...
echo "提交作业: sbatch job_script.sh"
...
echo ""
echo "5. 结果查看"
echo "查看输出文件: cat slurm-*.out"
echo "查看作业信息: scontrol show job JOB_ID"
EOF
...
echo "编译MPI程序: mpicc hello_mpi.c -o hello_mpi"
mpicc hello_mpi.c -o hello_mpi

echo "运行MPI程序: mpirun -np 4 ./hello_mpi"
mpirun -np 4 ./hello_mpi

# 2. OpenMP程序演示
echo ""
echo "2. OpenMP程序编译和运行"
cat > hello_omp.c << 'OMPEOF'
#include <omp.h>
#include <stdio.h>
...
echo "编译OpenMP程序: gcc -fopenmp hello_omp.c -o hello_omp"
gcc -fopenmp hello_omp.c -o hello_omp

echo "运行OpenMP程序: ./hello_omp"
...
echo ""
echo "3. 混合编程演示"
cat > hybrid_example.c << 'HYBRIDEOF'
#include <mpi.h>
#include <omp.h>
#include <stdio.h>
...
echo "编译混合程序: mpicc -fopenmp hybrid_example.c -o hybrid_example"
mpicc -fopenmp hybrid_example.c -o hybrid_example

echo "运行混合程序: mpirun -np 2 ./hybrid_example"
  ...
  "courses": [
    {
      "id": "hpc-basics",
      "name": "HPC基础入门",
      "description": "介绍高性能计算的基本概念和集群使用方法",
      "modules": [
        {
          "name": "HPC概述",
          "content": [
    ...
    },
    {
      "id": "slurm-basics",
      "name": "SLURM作业管理",
      "description": "学习SLURM作业调度系统的使用方法",
      "modules": [
        {
          "name": "SLURM基础",
          "content": [
...
class TicketAssigner:
    ...
    def select_agent(self, support_level, ticket_category):
        ...
        for agent in available_agents:
            ...
            load_penalty = agent['load'] * 0.1  # 负载惩罚
...
# 模拟查询超时工单（实际使用时替换为真实的API调用）
check_overdue_tickets() {
    echo "Checking for overdue tickets..."

    # 这里应该调用工单系统的API
    # 示例：curl -s "https://ticketsystem/api/tickets?status=open" | jq '.tickets[]'
...
class SLAMonitor:
    ...
    def generate_mock_data(self, hours):
        ...
        for i in range(50):
            ...
            # 随机生成响应时间
            if random.random() > 0.1:  # 90%的工单有响应
                response_delay = random.randint(1, 12)  # 1-12小时响应
                ticket['responded_at'] = (created_time + timedelta(hours=response_delay)).isoformat()

            # 随机生成解决时间
            if random.random() > 0.3:  # 70%的工单已解决
                resolution_delay = random.randint(2, 48)  # 2-48小时解决
...
collect_problem_info() {
    ...
    read -p "问题发生时间: " problem_time
    read -p "问题持续时间: " duration
    ...
    # 问题严重程度
    echo "请选择问题严重程度:"
    echo "1. Critical - 系统完全不可用"
    echo "2. High - 部分功能受影响"
    echo "3. Medium - 功能受限但可工作"
    echo "4. Low - 信息咨询类"
    read -p "请选择 (1-Critical, 2-High, 3-Medium, 4-Low): " severity
...
diagnose_account_issue() {
    ...
    if id $username &>/dev/null; then
        echo "✓ 用户 $username 存在"
        echo "用户信息:" >> /tmp/problem_$problem_id.txt
        id $username >> /tmp/problem_$problem_id.txt
    else
        echo "✗ 用户 $username 不存在"
        echo "建议: 请检查用户名是否正确或申请新账号" >> /tmp/problem_$problem_id.txt
        return 1
    fi
    ...
    if passwd -S $username | grep -q "Locked"; then
        echo "⚠️ 账号被锁定"
        echo "建议: 重置密码或联系管理员解锁" >> /tmp/problem_$problem_id.txt
    fi

    # 检查登录历史
    echo "最近登录记录:" >> /tmp/problem_$problem_id.txt
    last -n 5 $username >> /tmp/problem_$problem_id.txt

    # 检查配额
    echo "磁盘配额信息:" >> /tmp/problem_$problem_id.txt
...
diagnose_software_issue() {
    ...
    # 检查模块环境
    echo "当前模块环境:" >> /tmp/problem_$problem_id.txt
    module list >> /tmp/problem_$problem_id.txt 2>&1

    # 检查PATH
    echo "PATH环境变量:" >> /tmp/problem_$problem_id.txt
    echo $PATH >> /tmp/problem_$problem_id.txt

    # 检查常见软件
    echo "常用软件状态:" >> /tmp/problem_$problem_id.txt
    ...
    # 检查作业状态
    echo "当前作业状态:" >> /tmp/problem_$problem_id.txt
...
diagnose_network_issue() {
    ...
    # 检查网络连通性
    echo "网络连通性测试:" >> /tmp/problem_$problem_id.txt
    ...
    # 检查DNS解析
    echo "DNS解析测试:" >> /tmp/problem_$problem_id.txt
    nslookup google.com >> /tmp/problem_$problem_id.txt 2>&1

    # 检查网络接口
    echo "网络接口状态:" >> /tmp/problem_$problem_id.txt
    ip addr show >> /tmp/problem_$problem_id.txt

    # 检查路由表
    echo "路由表信息:" >> /tmp/problem_$problem_id.txt
...
generate_diagnostic_report() {
    ...
    if [ -f /tmp/problem_$problem_id.txt ]; then
        echo "诊断结果摘要:"
        grep -E "建议|推荐|问题|错误" /tmp/problem_$problem_id.txt | head -10
        echo ""

        echo "完整诊断报告保存在: /tmp/problem_$problem_id.txt"
    else
        echo "未找到诊断报告"
    fi
}
...
main_diagnosis() {
    ...
    echo ""
    echo "诊断流程完成"
    echo "请将问题ID $problem_id 提供给技术支持人员"
}
...
mpi_job_diagnosis() {
    ...
    if command -v mpirun &> /dev/null; then
        # 使用Intel VTune进行MPI分析（如果可用）
        if command -v vtune &> /dev/null; then
            echo "使用VTune进行MPI性能分析..."
...
infiniband_diagnosis() {
    ...
    if command -v ibstat &> /dev/null; then
        ...
        if command -v ib_send_bw &> /dev/null; then
            echo "InfiniBand带宽测试..."
            # 需要在两个节点上分别运行
            echo "服务器端: ib_send_bw -a"
            echo "客户端: ib_send_bw <server_ip>"
        fi
    else
        echo "InfiniBand工具未安装"
    fi
}
...
slurm_diagnosis() {
    ...
    echo "SLURM统计信息:"
    sstat -j $(squeue -h -t RUNNING -o "%i" | head -1) --format=JobID,MaxRSS,MaxVMSize,AveRSS,AveVMSize 2>/dev/null || echo "无法获取作业统计信息"
    ...
    while true; do
        ...
        case $choice in
            ...
            3)
                echo ""
                echo "技术支持联系方式:"
                echo "电话: 400-xxx-xxxx"
                echo "邮箱: support@hpc.example.com"
                echo "工单系统: https://support.hpc.example.com"
                ;;
            4)
                echo "感谢使用HPC支持系统"
                break
...
check_performance_thresholds() {
    ...
    if [ "$disk_usage" -gt "$DISK_THRESHOLD" ]; then
        echo "⚠️  Disk usage exceeds threshold: $disk_usage% > $DISK_THRESHOLD%"
...
send_alert() {
    ...
    if command -v curl &> /dev/null; then
        curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"🚨 HPC Alert: $alert_message ($alert_type: $alert_value)\"}" \
            https://hooks.slack.com/services/YOUR/SLACK/WEBHOOK
    fi
